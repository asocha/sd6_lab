...
const static float TWO_PI = 3.1415926535897932384626433832795f * 2.0f;
...
Vec2 GetPseudoRandomNoiseUnitVec2D( int positionX, int positionY );
float ComputePerlinNoiseValueAtPosition2D( const Vec2& position, float perlinNoiseGridCellSize, int numOctaves, float baseAmplitude, float persistance );
...
//---------------------------------------------------------------------------
inline Vec2 GetPseudoRandomNoiseUnitVec2D( int xPosition, int yPosition )
{
//	const float ONE_OVER_MAX_POSITIVE_INT = (1.f / 2147483648.f);
//	const float SCALE_FACTOR = ONE_OVER_MAX_POSITIVE_INT * TWO_PI;
	...
	return Vec2( cosineLookup256[ randomByte ], sineLookup256[ randomByte ] );
//	float pseudoRandomFloatZeroToTwoPi = SCALE_FACTOR * (float) pseudoRandomPositiveInt;
